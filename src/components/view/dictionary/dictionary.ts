    ...
    if (typeof words === 'string') {
      ...
      items = words
        .map((word: Word | WordUnderscore) => {
          return `<div class='word-card ${(word as WordUnderscore)?.userWord?.optional?.isLearned ? 'learned' : ''} ${
            (word as WordUnderscore)?.userWord?.difficulty === 'hard' ? 'hard' : ''
          }' data-id=${(word as Word).id ? (word as Word).id : (word as WordUnderscore)._id}>
